//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace IOAS.DataModel
{
    using System;
    using System.Collections.Generic;
    
    public partial class tblGeneralVoucherPaymentBreakUpDetail
    {
        public int GeneralVoucherPaymentBreakUpDetailId { get; set; }
        public Nullable<int> CategoryId { get; set; }
        public Nullable<int> UserId { get; set; }
        public string Name { get; set; }
        public Nullable<int> ModeOfPayment { get; set; }
        public Nullable<decimal> PaymentAmount { get; set; }
        public string Status { get; set; }
        public Nullable<int> UPDT_By { get; set; }
        public Nullable<System.DateTime> CRTD_TS { get; set; }
        public Nullable<System.DateTime> UPDT_TS { get; set; }
        public Nullable<int> CRTD_By { get; set; }
        public Nullable<int> GeneralVoucherId { get; set; }
        public Nullable<bool> IsHaveTDS { get; set; }
        public Nullable<int> TDSSection { get; set; }
        public Nullable<decimal> TDSAmount { get; set; }
        public Nullable<decimal> TDSPayable { get; set; }
    }
}
